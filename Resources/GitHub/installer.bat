...
cls
title Scubapaid Menu Installer // [#---------] Getting directory
color 0e
...
color 0e
cls
title Scubapaid Menu Installer // [###-------] Downloading BepInEx
...
cls
title Scubapaid Menu Installer // [####------] Creating directories
mkdir %gamePath%/BepInEx/config
mkdir %gamePath%/BepInEx/plugins

cls
title Scubapaid Menu Installer // [#####-----] Downloading latest config
curl https://raw.githubusercontent.com/Scubapaid/ModInfo/refs/heads/main/BepInEx.cfg -o %gamePath%/BepInEx/config/BepInEx.cfg

cls
title Scubapaid Menu Installer // [#######---] Downloading menu
for /f "tokens=*" %%i in ('powershell -Command "(Invoke-RestMethod -Uri 'https://api.github.com/repos/Scubapaid/Scubapaid-Menu/releases/latest').assets | Where-Object { $_.name -like '*.dll' } | Select-Object -ExpandProperty browser_download_url"') do (
    set pluginUrl=%%i
)
...
color 0e
curl -L "%pluginUrl%" -o %gamePath%/BepInEx/plugins/"Scubapaid Menu.dll"

cls
title Scubapaid Menu Installer // [##########] Finished
